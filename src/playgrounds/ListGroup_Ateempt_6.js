...
const ListGroup_Ateempt_6 = ({ }) => {
    const result = records.reduce((acc, cur, index) => {
        ...
        if (index < records.length - 1 && date !== records[index + 1].doneAt.slice(0, 10)) {
            const nextDate = acc[date + 1]?.lastTime;
            acc[`${date}-break`] = {
                startTime: acc[date].lastTime,
                endTime:
                    nextDate in acc
                        ? acc[nextDate].firstTime
                        : records[records.length - (index === records.length - 1 ? 1 : 2)].doneAt,
            };
        }
...
export default ListGroup_Ateempt_6;
